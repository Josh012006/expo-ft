...
class EXPOLearner(AgentLearner, struct.PyTreeNode):
    ...
    def create(
        ...
        # categorical-critic EXPOLearnerCategorical in expo_ft_categorical.py,
        # adapted here for REDQ's ensemble-subsampling
        # (subsample_image_ensemble) instead of a single critic head.
    ...
    def _update_finalize_jit(self, agent, last_minibatch, actor_batch: DatasetDict = None, use_success_batch: bool = False):
        ...
        Reverted from a jax.lax.cond version: that avoided a real but
        secondary risk (use_success_batch flipping True the first time a
        successful episode enters the buffer used to force a second,
        separately-resident compiled program for this function) at the cost
        of a bigger single compiled program (confirmed directly: rematerialization's
        own reported pre-reduction memory footprint went from ~34GiB to
        ~49.74GiB with lax.cond). More importantly, neither version was ever
        the actual cause of the OOM crashes observed on resume -- both died
        on the very first call to agent.update() overall (gated by
        `training_log.ep_count >= 10` in train_pi_robo.py's main loop, which
        resets to 0 on every resume regardless of prior training -- see that
        file), before use_success_batch could even become True. Reverting
        this specific change since it never addressed the real bottleneck
        and measurably increased first-compile memory pressure, which is
        exactly what's tight on a resumed run."""
        new_agent = agent
