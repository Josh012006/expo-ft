...
def evaluate(cfg, checkpoint_path, n_episodes, seed, video_dir=None, collect_action_stats=False,
    ...
    # Dispatch on cfg.model_cls rather than hardcoding the MSE
    # architecture's load_agent — evaluating an EXPOLearnerCategorical-trained
    ...
    # batch_stats vs. the MSE scalar/ensemble critic) before ever touching
    # the checkpoint on disk.
    if getattr(cfg, "model_cls", "EXPOLearner") == "EXPOLearnerCategorical":
        from expo_ft.agents.alg.expo_ft_categorical import load_agent
    else:
    ...
    if rl_checkpoint_path is not None:
        ...
        if getattr(cfg, "model_cls", "EXPOLearner") == "EXPOLearnerCategorical":
            from expo_ft.agents.alg.expo_ft_categorical import restore_checkpoint
        else:
